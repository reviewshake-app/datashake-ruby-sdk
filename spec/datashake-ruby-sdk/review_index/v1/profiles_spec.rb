...
RSpec.describe Datashake::ReviewIndex::V1::Profiles do
  let(:client) do
  ...
  context "when request timeouts" do
    it "throwns an error" do
      ...
      allow(connection_mock).to receive(:get).and_raise(Faraday::TimeoutError)
      ...
      version = Datashake::ReviewIndex::V1.new(client)
  ...
  context "when token is invalid" do
    it "throwns an error", :aggregate_failures do
      VCR.use_cassette("review_index/v1/profiles/invalid_token") do
        expect do
          subject.callback("https://xmpl.com/cb").name("KFC").fetch
        end
  ...
  context "when token is valid" do
    context "and required params are given" do
      it "returns 201" do
        VCR.use_cassette("review_index/v1/profiles/with_callback_and_name") do
    ...
    context "but no required param is given" do
      it "throws an error", :aggregate_failures do
        VCR.use_cassette("review_index/v1/profiles/no_callback") do
        ...
        VCR.use_cassette("review_index/v1/profiles/no_name") do
    ...
    context "and optional params are given" do
      context "when options are ok" do
        it "returns 201", :aggregate_failures do
          VCR.use_cassette("review_index/v1/profiles/with_options") do
            response =
              subject
      ...
      context "when trying to use both update and pagination" do
        it "throws an error", :aggregate_failures do
          VCR.use_cassette("review_index/v1/profiles/update_on_pagination") do
            expect do
